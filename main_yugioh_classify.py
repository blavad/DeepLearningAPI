...
flags.DEFINE_integer("input_height", 100, "The size of image to use (will be center cropped). [108]")
...
flags.DEFINE_string("model_name", "YuGiOh_Classifier_VGG", "The name of model (and checkpoint dir)")
...
flags.DEFINE_integer("skip_step", 10, "Number of step without display training parameters & save chackpoint . [108]")
FLAGS = flags.FLAGS

def label_processing(dataset):
    y = []
    for i in dataset :
        y.append([1.0,0.0]) if i.find('IMG')==-1 else y.append([0.0,1.0])
...
def main(_):
    ...
    data = ImageDataset(regex_file = data_path, input_height = FLAGS.input_height, input_width = FLAGS.input_width, as_line = False)
    ...
    with tf.Session() as sess :
        ...
        net = VGG([FLAGS.input_height, FLAGS.input_width, 3], 2,
                  learning_rate = FLAGS.learning_rate, build = True,
                  name = 'VGGNeuralNet')
